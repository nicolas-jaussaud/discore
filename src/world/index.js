...
const add = (app, name, args, callback) => {
  app.world.promises.push(
    new Promise(resolve =>{
      app.loaders.load(
        name,
        object => {
          callback( app.world.registerObject(object, args) )
          resolve()
      })
    })
  )
...
const registerObject = (app, object, args = []) => {
  ...
  const action = () => {
    ...
    /**
     * If object is not walkable, the path map is not valid anymore and needs
     * to be regenarated
     */
    if ( object.walkable === true ) {
      app.world.cache.delete(`_path-map-${app.map.current.name}`)
    }
